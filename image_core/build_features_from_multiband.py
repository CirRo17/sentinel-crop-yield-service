"""从本地多波段 GeoTIFF 构建标准特征栈。

该模块接收一张或多张已经对齐到同一网格的多波段影像，将波段序号
映射到 blue、green、red、rededge、nir 等语义波段，计算光谱指数，
并写出训练、离线预测和 API 共用的标准特征栈。
"""
...
def parse_band_map(value: str) -> dict[str, int]:
    ...
    if missing:
        raise argparse.ArgumentTypeError(f"波段映射缺少必要波段：{', '.join(missing)}")
    return mapping
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="可选 AOI 矢量文件，支持 GeoJSON/JSON；若安装 geopandas，也支持 Shapefile 等格式。",
    )
    parser.add_argument(
        "--geometry-crs",
        default="EPSG:4326",
