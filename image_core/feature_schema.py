...
模型训练、离线预测和 API 推理都通过这里检查特征名称，确保输入栅格
的 band 顺序与模型需要的特征顺序一致。
"""
