"""Step 10: 估产 — 基于分类结果和多光谱波段估算作物产量。

核心估产逻辑源自 crop_yield 模块（yield_model.py + config.py），
内嵌在本文件中，不依赖外部 crop_yield 目录。

使用方式:
    python -m pipeline.10_yield_estimation \
...
# ===================================================================
# 估产核心函数（原 crop_yield/yield_model.py，函数体一字不改）
# ===================================================================
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="step 06/07 输出的分类栅格。",
    )
    parser.add_argument(
        "--feature-stack",
        type=Path,
        default=DEFAULT_FEATURE_STACK,
        help="step 03b 输出的多时相特征栈。",
    )
    parser.add_argument(
        "--metadata",
        type=Path,
    ...
    parser.add_argument(
        ...
        choices=("ndvi", "lai"),
        default="ndvi",
        help="产量估算所用的植被指数。ndvi 取所有时相的逐像素均值；lai 需指定 --timepoint。",
    )
    parser.add_argument(
        "--timepoint",
        default=None,
        help="LAI 模式必须指定时相前缀（如 t2）。NDVI 模式忽略此参数。",
    )
    parser.add_argument(
        "--yield-function",
        default="default",
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="LAI 模型参数 k（覆盖默认值）。",
    )
    parser.add_argument(
        "--lai-m",
        type=float,
        default=None,
        help="LAI 模型参数 m（覆盖默认值）。",
    )
...
def _timepoint_prefixes(band_names: list[str]) -> list[str]:
    """从波段名列表提取所有唯一时相前缀（如 t1, t2, 2025_07 等）。"""
    seen: dict[str, bool] = {}
    prefixes: list[str] = []
...
def _select_timepoint(
    ...
    若用户指定了 timepoint 则直接使用；否则在所有时相中选
    作物区域 NDVI 均值最高的那个。
    """
    ...
    if specified is not None:
        if specified not in prefixes:
            raise ValueError(
                f"指定的时相 '{specified}' 不在特征栈中。可用时相: {', '.join(prefixes)}"
            )
        return specified
...
def main() -> None:
    ...
    with rasterio.open(args.feature_stack) as fs_src:
        if args.index == "lai":
            ...
            if any(i is None for i in (red_idx, nir_idx, rededge_idx)):
                raise ValueError(
                    f"时相 {args.timepoint} 缺少 LAI 所需的 red/nir/rededge 波段。"
                )
            red = fs_src.read(red_idx, masked=False).astype("float32")
    ...
    combined_yield = np.full(
        (class_profile["height"], class_profile["width"]), np.nan, dtype="float32"
    )
    ...
    for code in sorted(SUPPORTED_CODES):
        ...
        # 确定该作物的预测因子
        if args.index == "lai" and ci is not None:
            # 逐作物计算 LAI（k、m 参数因作物而异）
            crop_predictor = lai_from_ci(ci, crop_name, args.lai_k, args.lai_m)
        else:
        ...
        try:
            pixel_yield = estimate_yield(
                ...
                function_type=args.yield_function,
            )
        ...
        # 写入分作物产量栅格
        crop_yield_raster = np.where(
            valid_yield, pixel_yield, yield_profile["nodata"]
        ).astype("float32")
        ...
        # 填充综合产量栅格
        combined_yield = np.where(
            valid_yield, pixel_yield, combined_yield
        )
        ...
        print(
            f"  {label}: 面积 {area_ha:.1f} ha, "
            f"均产 {mean_yield:.1f} kg/ha, "
            f"总产 {total_yield:.0f} kg"
        )

    # 6. 写入综合产量栅格
    combined_out = args.output_dir / "yield_all.tif"
    combined_data = np.where(
        np.isfinite(combined_yield), combined_yield, yield_profile["nodata"]
    ).astype("float32")
