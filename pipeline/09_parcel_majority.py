"""Step 09: generate parcel-level majority Shapefile.

本步骤把 step 06/07 生成的像素级分类图套到地块 Shapefile 上，
对每个地块统计内部占比最高的作物类别，并输出新的地块级 Shapefile。
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Attach raster majority crop class to parcel polygons.")
    parser.add_argument(
        "--classification",
        type=Path,
        default=DEFAULT_CLASSIFICATION,
        help="像素级分类 GeoTIFF，通常来自 step 06 或 step 07。",
    )
    parser.add_argument(
        "--parcels",
        type=Path,
        default=DEFAULT_PARCELS,
        help="地块 Shapefile。API 场景下由调用方上传；离线场景下手动指定。",
    )
    parser.add_argument(
        "--output-shp",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="输出 Shapefile 压缩包，方便接口下载或成果分发。",
    )
    parser.add_argument(
        "--summary",
        type=Path,
...
def _zip_shapefile(output_shp: Path, zip_path: Path) -> None:
    """把 Shapefile 的全部伴随文件打包，便于 API 或人工下载。"""
...
def main() -> None:
    ...
    if not args.classification.exists():
        raise FileNotFoundError(f"Missing classification raster: {args.classification}. Run step 06/07 first.")
    if not args.parcels.exists():
        raise FileNotFoundError(f"Missing parcel shapefile: {args.parcels}.")

    # 这里调用 pipeline 的处理函数；API 也调用同一个函数，避免两套地块统计逻辑。
