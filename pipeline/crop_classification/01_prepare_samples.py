...
输入可以是单个 GeoTIFF，也可以是包含多张 GeoTIFF 的目录。脚本会将
标签图按最近邻重采样到特征栈网格，然后抽取像素级训练样本，输出给
02_train_rf.py 使用。类别 0（Others）来自所有专题图之外的区域。
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max-per-class", type=int, default=5000, help="每类最大抽样像元数。")
...
def build_label_masks(
    ...
    # 清理跨类别重叠像元：同一像元若被多个作物类别命中，则归入 Others。
    ...
    if np.any(ambiguous):
        ...
        print(
            f"警告：{int(np.count_nonzero(ambiguous))} 个像元被多个作物类别同时命中，"
            "已从所有作物掩膜中移除，归入类别 0（Others）。"
        )
