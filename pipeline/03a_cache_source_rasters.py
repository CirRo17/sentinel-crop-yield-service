"""步骤 03a：缓存或落库与 AOI 对齐的 Sentinel-1/2 源栅格。

本步骤用于为 03_build_features.py 预热缓存。它读取 Sentinel-2 和可选的
Sentinel-1 清单，将选定的源影像资产对齐到项目 AOI 网格，并把对齐后的数组
保存到 data/exported/cache/。
...
from crop_classifier_core.config import S2_SCALE


# ---------------------------------------------------------------------------
# GDAL / PROJ 配置（与 03b 共用）
# ---------------------------------------------------------------------------
...
DEFAULT_S2_MANIFEST = Path("data/exported/sentinel2_scenes.json")
DEFAULT_S1_MANIFEST = Path("data/exported/sentinel1_scenes.json")
DEFAULT_CACHE_DIR = Path("data/exported/cache")
DEFAULT_LOCAL_SOURCE_DIR = Path("data/source/aws_local")
DEFAULT_LOCAL_S2_MANIFEST = Path("data/exported/sentinel2_scenes_local.json")
DEFAULT_LOCAL_S1_MANIFEST = Path("data/exported/sentinel1_scenes_local.json")
...
def read_asset_raw_on_grid(
    ...
    with rasterio.Env(
        ...
        with rasterio.open(normalize_href(href)) as src:
            # 注意：Sentinel-1 GRD 产品可能只有 GCP，而没有内嵌 CRS。
            # WarpedVRT 无法处理只有 GCP 的数据源。
            if src.crs is None and src.gcps:
                raise RuntimeError(
                    f"暂不支持 S1 这类仅含 GCP 的数据源。"
                    f"请使用仅 S2 模式（省略 --include-s1）。数据源：{href[:80]}..."
                )
            with WarpedVRT(
                src,
                crs=dst_crs,
...
def main() -> None:
    ...
    if args.materialize_local_assets:
        ...
        print("本地落库完成。请在步骤 03b 中使用 *_local.json 清单。", flush=True)
        return
