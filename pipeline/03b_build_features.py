"""步骤 03b：构建统一的多时相 Sentinel-1/2 标准特征栈。

这是当前 Sentinel 输入适配流程，后续可与无人机/本地多波段输入并存。
脚本读取 Sentinel-2 和可选 Sentinel-1 清单，按配置时相生成月度中值合成，
计算光谱指数和雷达特征，并在 data/exported/ 下写出多波段 GeoTIFF。

远程 AWS 读取结果会缓存为 AOI 对齐数组，存放在 data/exported/cache/。
可先运行 03a_cache_source_rasters.py 预热缓存或落库本地资产。
"""
...
from crop_classifier_core.config import CLOUD_SCL_VALUES
from crop_classifier_core.spectral import evi, nbr, ndre, ndvi, ndwi

# 共享的缓存/网格工具在 03a 中；因为模块名以数字开头，这里用 importlib 导入。
cache = importlib.import_module("pipeline.03a_cache_source_rasters")

# ---------------------------------------------------------------------------
# 03b 专用常量
# ---------------------------------------------------------------------------

DEFAULT_OUTPUT = Path("data/exported/feature_stack.tif")
...
def build_s2_timepoint_composite(
    ...
    for scene in scenes:
        print(f"Reading Sentinel-2 scene {scene.get('id', '<unknown>')} for {timepoint.get('label')}", flush=True)
        try:
            arrays = scene_to_s2_arrays(scene, dst_crs, transform, width, height, aoi_mask, cache_dir)
        except Exception as exc:
            print(f"Skipping Sentinel-2 scene {scene.get('id', '<unknown>')}: {str(exc)[:120]}", flush=True)
            continue
        if arrays is None:
            continue
    ...
    if not used_scene_ids:
        raise ValueError(f"No usable Sentinel-2 scenes for timepoint {timepoint.get('label')}")
...
def build_s1_timepoint_composite(
    ...
    scenes = timepoint.get("scenes", [])[:max_scenes] if max_scenes is not None else timepoint.get("scenes", [])
    for scene in scenes:
        print(f"Reading Sentinel-1 scene {scene.get('id', '<unknown>')} for {timepoint.get('label')}", flush=True)
        try:
            arrays = scene_to_s1_arrays(scene, dst_crs, transform, width, height, aoi_mask, cache_dir)
        except Exception as exc:
            print(f"Skipping Sentinel-1 scene {scene.get('id', '<unknown>')}: {str(exc)[:120]}", flush=True)
            continue
        if arrays is None:
            continue
        used_scene_ids.append(scene.get("id", ""))
        for name in S1_BANDS:
            scene_arrays[name].append(arrays[name])
    if not used_scene_ids:
        raise ValueError(f"No usable Sentinel-1 scenes for timepoint {timepoint.get('label')}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--timepoints", nargs="*", default=None, help="可选时相标签，例如 2025-04 2025-07")
    parser.add_argument(
        "--timepoint-name-mode",
        choices=("slot", "label"),
        default="slot",
...
def main() -> None:
    ...
    for timepoint_index, timepoint in enumerate(s2_timepoints, start=1):
        ...
        if args.include_s1:
            ...
            if not s1_timepoint or not s1_timepoint.get("scenes"):
                s1_metadata.append({"label": timepoint["label"], "used_scene_count": 0, "error": "No Sentinel-1 scenes for this timepoint"})
                continue
            try:
                ...
                print(f"Skipping Sentinel-1 for {timepoint['label']}: {str(exc)[:120]}", flush=True)

    write_feature_stack(args.output, band_arrays, band_names, dst_crs, transform)
    metadata = {
    ...
    print(f"Saved feature stack: {args.output}", flush=True)
    print(f"Saved metadata: {args.metadata}", flush=True)
