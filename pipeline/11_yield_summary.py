"""Step 11: 产量汇总 — 地块级产量统计与报告生成。

本步骤读取 step 10 的产量栅格和地块 Shapefile，
对每个地块统计各作物产量（总产/面积/单产），并生成汇总报告。
...
_yield = importlib.import_module("pipeline.10_yield_estimation")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="step 10 输出的 yield_stats.json。",
    )
    parser.add_argument(
        "--yield-raster",
        type=Path,
        default=DEFAULT_YIELD_RASTER,
        help="step 10 输出的综合产量栅格（地块模式需要）。",
    )
    parser.add_argument(
        "--classification",
        type=Path,
        ...
        with rasterio.open(classification_path) as class_src:
            for idx, row in gdf.iterrows():
                ...
                try:
                    ...
                    results.append({**props, "error": "classification read failed"})
                    continue
                ...
                try:
                    ...
                    results.append({**props, "error": "yield read failed"})
                    continue
...
def main() -> None:
    ...
    if args.parcels and args.parcels.exists():
        ...
        if not args.yield_raster.exists():
            ...
            parcel_results = _parcel_yield_stats(
                args.yield_raster, args.classification, args.parcels
            )
