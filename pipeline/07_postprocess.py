"""Step 07: postprocess classification outputs.

This step turns the raw step-06 prediction rasters into a cleaner, traceable
map product. It applies conservative rules only:

- convert low-confidence, invalid, and unknown pixels to output class 0
- optionally sieve small patches
- write a cleaned classification GeoTIFF and a JSON processing record
"""
...
def main() -> None:
    ...
    if not args.classification.exists():
        raise FileNotFoundError(f"Missing classification raster: {args.classification}. Run step 06 first.")
    if not args.confidence.exists():
        raise FileNotFoundError(f"Missing confidence raster: {args.confidence}. Run step 06 first.")
