...
from data_sources.sentinel.config import CLOUD_SCL_VALUES
from image_core.spectral import evi, nbr, ndre, ndvi, ndwi
from data_sources.sentinel import cache_source_rasters as cache

# 复用 Sentinel 源影像缓存和统一网格读取工具。
...
def main() -> None:
    ...
    print(f"已保存特征栈：{args.output}", flush=True)
    print(f"已保存元数据：{args.metadata}", flush=True)
